...
import (
	...
	aggregator "github.com/thehyperflames/hyperion-runtime/internal/aggregator"
	"github.com/thehyperflames/hyperion-api/internal/http/httputil"
)
...
func NewHandler(aggregatorSvc *aggregator.Service) *Handler {
	return &Handler{aggregatorSvc: aggregatorSvc}
}

func (h *Handler) SetRoutes(pub *gin.RouterGroup, private *gin.RouterGroup, admin *gin.RouterGroup) {
...
func (h *Handler) getPool(c *gin.Context) {
	...
	if pool == nil {
		httputil.HandleNotFound(c, "pool not found")
		return
	}

	httputil.HandleSuccess(c, PoolDetailResponse{
		Address:         pool.Address.String(),
		Type:            pool.Type.String(),
